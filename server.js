...
const app = express();
// Railway/reverse proxies send X-Forwarded-For. Trust the first proxy so
// express-rate-limit can safely determine the real client IP.
...
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  ...
  // Railway currently runs Node 20, which does not provide a native
  // WebSocket implementation required by Supabase Realtime.
...
// Serve the frontend assets before the API/fallback routes.
// Without this, requests such as /style.css and /app.js fall through to
// the HTML catch-all and are returned with text/html, breaking the UI.
...
async function ensureDatabaseSchema(){
  // The project may already contain an older `public.users` table. CREATE TABLE
  // IF NOT EXISTS does not add missing columns, so migrate the users table here
  // before the first request instead of crashing on registration/profile queries.
  await q(`ALTER TABLE public.users
    ADD COLUMN IF NOT EXISTS password_hash text,
    ADD COLUMN IF NOT EXISTS display_name text,
    ADD COLUMN IF NOT EXISTS avatar text,
    ADD COLUMN IF NOT EXISTS bio text,
    ADD COLUMN IF NOT EXISTS updated_at timestamptz`);
  // Older versions of Gish Chat used a legacy `password` column with a
  // NOT NULL constraint. Authentication now uses `password_hash`, so make
  // the legacy column nullable when it exists. This keeps old databases
  // compatible without storing plaintext passwords.
  await q(`DO $$
  BEGIN
    IF EXISTS (
    ...
    END IF;
  END $$;`);
  await q(`UPDATE public.users
    SET display_name = COALESCE(NULLIF(display_name, ''), username),
        avatar = COALESCE(avatar, ''),
        bio = COALESCE(bio, ''),
        updated_at = COALESCE(updated_at, created_at, now())
    WHERE display_name IS NULL OR display_name = '' OR avatar IS NULL OR bio IS NULL OR updated_at IS NULL`);
  await q(`ALTER TABLE public.users
