...
async function ensureDatabaseSchema() {
  ...
  `);
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS conversations_channel_username_uq ON conversations(username) WHERE type='channel' AND username IS NOT NULL`);
...
app.patch('/api/conversations/:id', auth, async (req,res)=>{
  ...
  const r=await q(`UPDATE conversations SET name=$1,description=$2,username=CASE WHEN type='channel' THEN NULLIF($3,'') ELSE username END,updated_at=now() WHERE id=$4 RETURNING *`,[name,desc,username,cid]); res.json(await conversationView(r.rows[0]));
});
