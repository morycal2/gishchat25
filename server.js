...
function corsOrigin(origin, cb) { cb(null, true); }
const io = new Server(server, {
  cors: { origin: true, methods: ['GET', 'POST'], credentials: false },
  transports: ['polling','websocket'], pingInterval: 25000, pingTimeout: 20000
});
...
async function ensureDatabaseSchema() {
  ...
  await q(`ALTER TABLE messages ADD COLUMN IF NOT EXISTS duration_ms integer NOT NULL DEFAULT 0`);
  await q(`CREATE TABLE IF NOT EXISTS user_settings (user_id bigint PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE, data jsonb NOT NULL DEFAULT '{}'::jsonb, updated_at timestamptz NOT NULL DEFAULT now())`);
...
async function conversationView(c) {
  ...
  const m = last.rows[0];
  let lastText = '';
  if (m) lastText = m.kind === 'voice' ? '🎙️ پیام صوتی' : m.kind === 'video_message' ? '🎥 ویدیو مسیج' : m.kind === 'image' ? '🖼️ تصویر' : m.kind === 'video' ? '🎬 ویدیو' : m.kind === 'audio' ? '🎵 آهنگ' : (m.text || '📎 فایل');
  return {
...
async function insertMessage({ cid, uid, text, kind='text', fileUrl='', fileType='', fileName='', replyTo=null, durationMs=0 }) {
  const duration = Math.max(0, Math.min(3600000, Number(durationMs)||0));
  const r = await q(`INSERT INTO messages(conversation_id,sender_id,text,file_url,file_type,file_name,kind,reply_to,duration_ms)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`, [cid, uid, text, fileUrl, fileType, fileName, kind, replyTo, duration]);
...
app.post('/api/messages', auth, async (req, res) => {
  try {
    ...
    const fileUrl = String(req.body.fileUrl || '').trim().slice(0, 2000);
    const fileType = String(req.body.fileType || '').trim().slice(0, 160);
    const fileName = safeFileName(req.body.fileName || '');
    const kind = String(req.body.kind || (fileUrl ? 'file' : 'text')).slice(0, 40);
    const replyTo = req.body.replyTo ? Number(req.body.replyTo) : null;
    const durationMs = Math.max(0, Math.min(3600000, Number(req.body.durationMs)||0));
    if (!text && !fileUrl) return res.status(400).json({ error: 'پیام خالی است' });
    const out = await insertMessage({ cid, uid: req.user.id, text, fileUrl, fileType, fileName, kind, replyTo, durationMs });
...
const DEFAULT_SETTINGS={notifications:{messages:true,sounds:true,previews:true},data_usage:{autoplay:true,autoDownloadImages:true,autoDownloadVideos:false,autoDownloadAudio:false},privacy:{lastSeen:'everyone',profilePhoto:'everyone',readReceipts:true},security:{twoStep:false},appearance:{dark:false,compact:false}};
app.get('/api/settings', auth, async (req,res)=>{try{const r=await q('SELECT data FROM user_settings WHERE user_id=$1',[req.user.id]);const d=r.rows[0]?.data||{};res.json({...DEFAULT_SETTINGS,...d,notifications:{...DEFAULT_SETTINGS.notifications,...(d.notifications||{})},data_usage:{...DEFAULT_SETTINGS.data_usage,...(d.data_usage||{})},privacy:{...DEFAULT_SETTINGS.privacy,...(d.privacy||{})},security:{...DEFAULT_SETTINGS.security,...(d.security||{})},appearance:{...DEFAULT_SETTINGS.appearance,...(d.appearance||{})}})}catch(e){console.error(e);res.status(500).json({error:'دریافت تنظیمات ناموفق بود'})}});
app.put('/api/settings', auth, async (req,res)=>{try{const data={...DEFAULT_SETTINGS,...(req.body||{})};await q(`INSERT INTO user_settings(user_id,data,updated_at) VALUES($1,$2,now()) ON CONFLICT(user_id) DO UPDATE SET data=EXCLUDED.data,updated_at=now()`,[req.user.id,JSON.stringify(data)]);res.json(data)}catch(e){console.error(e);res.status(500).json({error:'ذخیره تنظیمات ناموفق بود'})}});
app.delete('/api/account', auth, async (req,res)=>{try{await q('DELETE FROM users WHERE id=$1',[req.user.id]);res.json({ok:true})}catch(e){console.error(e);res.status(500).json({error:'حذف حساب ناموفق بود'})}});
...
io.on('connection', socket => {
  ...
  socket.on('send_message', async d => {
    try { const cid=Number(d.conversationId), check=await canMessage(cid,uid); if(!check.ok)return;
      const text=String(d.text||'').trim().slice(0,5000), fileUrl=String(d.fileUrl||'').slice(0,2000), fileType=String(d.fileType||'').slice(0,160), fileName=safeFileName(d.fileName||'');
      const durationMs=Math.max(0,Math.min(3600000,Number(d.durationMs)||0));
      if(!text&&!fileUrl)return; const out=await insertMessage({cid,uid,text,kind:String(d.kind||'text').slice(0,40),fileUrl,fileType,fileName,replyTo:d.replyTo?Number(d.replyTo):null,durationMs}); io.to('conv:'+cid).emit('message',out);
