...
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  ...
  // Supabase Realtime on Node.js 20 needs an explicit WebSocket transport.
...
// Serve the bundled frontend assets (CSS/JS/images/manifest) before the API and SPA fallback.
// Without this, requests such as /style.css or /app.js fall through to index.html and the browser
// rejects them because they arrive with text/html instead of the correct MIME type.
...
// Keep the production database compatible with older Gish Chat installations.
// The project originally used a smaller `users` table; the current API also needs
// profile fields and password_hash. These idempotent migrations make deployment
// safe to repeat and create the remaining app tables when they do not exist yet.
async function ensureDatabaseSchema() {
  ...
  // Legacy installations may have stored the password in a column named password.
  // We do not use it for new accounts; login below can upgrade a legacy value.
  await q(`ALTER TABLE users ADD COLUMN IF NOT EXISTS password text`);
  // Older databases may have password marked NOT NULL. New accounts use
  // password_hash, so the legacy password column must be allowed to stay NULL.
  ...
  `);
  await q(`CREATE UNIQUE INDEX IF NOT EXISTS conversations_username_uq ON conversations(username) WHERE username IS NOT NULL`);
  ...
  `);
  await q(`CREATE INDEX IF NOT EXISTS messages_conversation_id_id_idx ON messages(conversation_id,id DESC)`);
  ...
  `);
  await q(`ALTER TABLE conversation_members ADD COLUMN IF NOT EXISTS role text NOT NULL DEFAULT 'member'`);
  await q(`ALTER TABLE conversations ADD COLUMN IF NOT EXISTS photo text NOT NULL DEFAULT ''`);

  // Backfill owners as admins for existing groups/channels.
  await q(`INSERT INTO conversation_admins(conversation_id,user_id,role,permissions)
    SELECT id, owner_id, 'owner', '{"post":true,"edit":true,"delete":true,"invite":true,"ban":true,"manage":true}'::jsonb
    FROM conversations WHERE owner_id IS NOT NULL AND type IN ('group','channel')
    ON CONFLICT (conversation_id,user_id) DO UPDATE SET role='owner', permissions=EXCLUDED.permissions`);
}
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    let valid = false;
    if (u?.password_hash) {
      valid = await bcrypt.compare(supplied, u.password_hash);
    } else if (u?.password) {
      // One-time compatibility for old databases. If the old value is plaintext,
      // immediately replace it with a bcrypt hash after a successful login.
      if (/^\$2[aby]\$\d{2}\$/.test(String(u.password))) {
        valid = await bcrypt.compare(supplied, u.password);
      } else if (supplied === String(u.password)) {
        valid = true;
        const hash = await bcrypt.hash(supplied, 12);
        await q('UPDATE users SET password_hash=$1, password=NULL, updated_at=now() WHERE id=$2', [hash, u.id]);
...
async function getAdmin(cid, uid) {
  const r = await q('SELECT * FROM conversation_admins WHERE conversation_id=$1 AND user_id=$2', [Number(cid), Number(uid)]);
  return r.rows[0] || null;
}
async function requireManager(req, res, cid, permission='manage') {
  const c = await getConversation(cid);
  if (!c || !['group','channel'].includes(c.type)) { res.status(404).json({error:'گروه یا کانال پیدا نشد'}); return null; }
  const a = await getAdmin(cid, req.user.id);
  if (!a || !(a.permissions?.[permission] ?? false)) { res.status(403).json({error:'دسترسی کافی ندارید'}); return null; }
  return c;
}
...
app.post('/api/conversations/group', auth, async (req, res) => {
  ...
  if (username && (await q('SELECT 1 FROM conversations WHERE username=$1',[username])).rowCount) return res.status(409).json({error:'این شناسه قبلاً استفاده شده است'});
  ...
  await q(`INSERT INTO conversation_admins(conversation_id,user_id,role,permissions) VALUES($1,$2,'owner','{"post":true,"edit":true,"delete":true,"invite":true,"ban":true,"manage":true}') ON CONFLICT DO NOTHING`, [c.id, req.user.id]);
  res.json(await conversationView(c));
});
...
app.get('/api/public/conversations/:username', async (req,res)=>{
  try { const username=String(req.params.username||'').trim().toLowerCase(); const r=await q(
    `SELECT id,name,type,username,description,photo,created_at FROM conversations WHERE username=$1 AND type IN ('group','channel') LIMIT 1`,[username]);
    if(!r.rowCount)return res.status(404).json({error:'گروه یا کانال پیدا نشد'});
    const c=r.rows[0]; const members=await q('SELECT count(*)::int AS count FROM conversation_members WHERE conversation_id=$1',[c.id]);
    res.json({...c,id:Number(c.id),members_count:members.rows[0].count,invite_link:`${req.protocol}://${req.get('host')}/#/join/${encodeURIComponent(c.username)}`});
  } catch(e){console.error(e);res.status(500).json({error:'خطا در دریافت لینک'})}
});

app.post('/api/conversations/:id/join', auth, async (req, res) => {
  const c = await getConversation(req.params.id);
  if (!c || !['channel','group'].includes(c.type)) return res.status(404).json({ error: 'گروه یا کانال پیدا نشد' });
...
async function canMessage(cid, uid) {
  ...
  if (c.type === 'channel') {
    const a = await getAdmin(cid, uid);
    if (!a || !(a.permissions?.post ?? false)) return { ok: false, error: 'دسترسی ارسال در این کانال را ندارید' };
  }
  if (c.type === 'group') {
    const a = await getAdmin(cid, uid);
    if (a && a.permissions && a.permissions.post === false) return { ok: false, error: 'دسترسی ارسال شما محدود شده است' };
  }
  if (c.type === 'direct') {
...
app.post('/api/messages', auth, async (req, res) => {
  try {
    ...
    if (!text && !req.body.fileUrl) return res.status(400).json({ error: 'پیام خالی است' });
...
app.get('/api/conversations/:id/admins', auth, async (req,res)=>{
  const cid=Number(req.params.id); if(!await isMember(cid,req.user.id)) return res.status(403).json({error:'عضو گفتگو نیستید'});
  const r=await q(`SELECT ca.user_id,ca.role,ca.permissions,ca.created_at,u.username,u.display_name,u.avatar FROM conversation_admins ca JOIN users u ON u.id=ca.user_id WHERE ca.conversation_id=$1 ORDER BY CASE ca.role WHEN 'owner' THEN 0 WHEN 'admin' THEN 1 ELSE 2 END,u.display_name`,[cid]);
  res.json(r.rows.map(x=>({...x,user_id:Number(x.user_id)})));
});
app.post('/api/conversations/:id/admins', auth, async (req,res)=>{
  const cid=Number(req.params.id), uid=Number(req.body.userId); const c=await requireManager(req,res,cid,'manage'); if(!c)return;
  if(!await isMember(cid,uid)) return res.status(400).json({error:'کاربر باید عضو گفتگو باشد'});
  if(uid===Number(c.owner_id)) return res.status(400).json({error:'مالک را نمی‌توان تغییر داد'});
  const role=['admin','moderator'].includes(req.body.role)?req.body.role:'admin';
  const permissions={post:true,edit:true,delete:true,invite:true,ban:false,manage:false,...(req.body.permissions||{})};
  await q(`INSERT INTO conversation_admins(conversation_id,user_id,role,permissions) VALUES($1,$2,$3,$4) ON CONFLICT(conversation_id,user_id) DO UPDATE SET role=EXCLUDED.role,permissions=EXCLUDED.permissions`,[cid,uid,role,JSON.stringify(permissions)]);
  res.json({ok:true});
});
app.patch('/api/conversations/:id/admins/:userId', auth, async (req,res)=>{
  const cid=Number(req.params.id), uid=Number(req.params.userId); const c=await requireManager(req,res,cid,'manage'); if(!c)return;
  if(uid===Number(c.owner_id)) return res.status(400).json({error:'مالک قابل ویرایش نیست'});
  const permissions={post:!!req.body.post,edit:!!req.body.edit,delete:!!req.body.delete,invite:!!req.body.invite,ban:!!req.body.ban,manage:!!req.body.manage};
  await q('UPDATE conversation_admins SET permissions=$1,role=$2 WHERE conversation_id=$3 AND user_id=$4',[JSON.stringify(permissions),['admin','moderator'].includes(req.body.role)?req.body.role:'admin',cid,uid]); res.json({ok:true});
});
app.delete('/api/conversations/:id/admins/:userId', auth, async (req,res)=>{
  const cid=Number(req.params.id), uid=Number(req.params.userId); const c=await requireManager(req,res,cid,'manage'); if(!c)return;
  if(uid===Number(c.owner_id)) return res.status(400).json({error:'مالک قابل حذف نیست'});
  await q('DELETE FROM conversation_admins WHERE conversation_id=$1 AND user_id=$2',[cid,uid]); res.json({ok:true});
});
app.delete('/api/conversations/:id/members/:userId', auth, async (req,res)=>{
  const cid=Number(req.params.id), uid=Number(req.params.userId); const c=await requireManager(req,res,cid,'ban'); if(!c)return;
  if(uid===Number(c.owner_id)) return res.status(400).json({error:'مالک قابل حذف نیست'});
  await q('DELETE FROM conversation_members WHERE conversation_id=$1 AND user_id=$2',[cid,uid]); await q('DELETE FROM conversation_admins WHERE conversation_id=$1 AND user_id=$2',[cid,uid]);
  io.to('conv:'+cid).emit('member_removed',{conversationId:cid,userId:uid}); res.json({ok:true});
});
app.patch('/api/conversations/:id', auth, async (req,res)=>{
  const cid=Number(req.params.id); const c=await requireManager(req,res,cid,'manage'); if(!c)return;
  const name=String(req.body.name??c.name).trim().slice(0,60), desc=String(req.body.description??c.description).trim().slice(0,500), username=String(req.body.username??c.username??'').trim().toLowerCase().replace(/[^a-z0-9_]/g,'').slice(0,30);
  const r=await q(`UPDATE conversations SET name=$1,description=$2,username=NULLIF($3,''),updated_at=now() WHERE id=$4 RETURNING *`,[name,desc,username,cid]); res.json(await conversationView(r.rows[0]));
});

app.get('/api/settings', auth, async (req,res)=>{
  await q('INSERT INTO user_settings(user_id) VALUES($1) ON CONFLICT DO NOTHING',[req.user.id]);
  const r=await q('SELECT notifications,data_usage,privacy,security FROM user_settings WHERE user_id=$1',[req.user.id]); res.json(r.rows[0]);
});
app.put('/api/settings', auth, async (req,res)=>{
  const body=req.body||{}; await q(`INSERT INTO user_settings(user_id,notifications,data_usage,privacy,security) VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id) DO UPDATE SET notifications=EXCLUDED.notifications,data_usage=EXCLUDED.data_usage,privacy=EXCLUDED.privacy,security=EXCLUDED.security,updated_at=now()`,[req.user.id,JSON.stringify(body.notifications||{}),JSON.stringify(body.data_usage||{}),JSON.stringify(body.privacy||{}),JSON.stringify(body.security||{})]); res.json({ok:true});
});

app.get('/api/media/download/:messageId', auth, async (req,res)=>{
  try { const mid=Number(req.params.messageId); const r=await q('SELECT * FROM messages WHERE id=$1',[mid]); const m=r.rows[0]; if(!m||!await isMember(m.conversation_id,req.user.id)) return res.status(404).send('Not found');
    if(!m.file_url) return res.status(404).send('No media'); const rr=await fetch(m.file_url); if(!rr.ok) return res.status(502).send('Media unavailable');
    res.setHeader('Content-Type',m.file_type||rr.headers.get('content-type')||'application/octet-stream'); res.setHeader('Content-Disposition',`attachment; filename*=UTF-8''${encodeURIComponent(safeFileName(m.file_name||'gish-file'))}`);
    const buf=Buffer.from(await rr.arrayBuffer()); res.send(buf);
...
app.put('/api/profile', auth, async (req, res) => {
  ...
  const username = String(req.body.username || req.user.username).trim().toLowerCase().replace(/[^a-z0-9_]/g,'').slice(0,30);
  if (!name || username.length < 3) return res.status(400).json({ error: 'نام و شناسه معتبر لازم است' });
  const taken=await q('SELECT 1 FROM users WHERE username=$1 AND id<>$2',[username,req.user.id]);
  if(taken.rowCount)return res.status(409).json({error:'این شناسه قبلاً استفاده شده است'});
  const r = await q('UPDATE users SET username=$1,display_name=$2,bio=$3,updated_at=now() WHERE id=$4 RETURNING id,username,email,display_name,avatar,bio', [username,name,bio,req.user.id]);
  res.json(safeUser(r.rows[0]));
});
app.delete('/api/account', auth, async (req,res)=>{
  const client=await pool.connect();
  try{ await client.query('BEGIN'); await client.query('DELETE FROM users WHERE id=$1',[req.user.id]); await client.query('COMMIT'); res.json({ok:true}); }
  catch(e){await client.query('ROLLBACK'); console.error(e); res.status(500).json({error:'حذف حساب ناموفق بود'});} finally{client.release();}
});
...
async function getSavedConversation(uid){
  let r=await q(`SELECT * FROM conversations WHERE type='direct' AND owner_id=$1 AND name='پیام‌های ذخیره‌شده' LIMIT 1`,[uid]);
  if(r.rowCount)return r.rows[0];
  r=await q(`INSERT INTO conversations(name,type,owner_id,username,description) VALUES('پیام‌های ذخیره‌شده','direct',$1,NULL,'فضای شخصی') RETURNING *`,[uid]);
  await q('INSERT INTO conversation_members(conversation_id,user_id) VALUES($1,$2)',[r.rows[0].id,uid]); return r.rows[0];
}
app.get('/api/saved/chat',auth,async(req,res)=>{const c=await getSavedConversation(req.user.id);const r=await q(`SELECT * FROM messages WHERE conversation_id=$1 ORDER BY id ASC LIMIT 500`,[c.id]);const out=[];for(const m of r.rows)out.push(await messageView(m));res.json({conversation:{id:Number(c.id),name:c.name,type:'saved',owner_id:req.user.id},messages:out});});
app.post('/api/saved/chat/messages',auth,async(req,res)=>{try{const c=await getSavedConversation(req.user.id);const text=String(req.body.text||'').trim().slice(0,5000),fileUrl=String(req.body.fileUrl||'').slice(0,1000),fileType=String(req.body.fileType||'').slice(0,120),fileName=safeFileName(req.body.fileName||'');if(!text&&!fileUrl)return res.status(400).json({error:'پیام خالی است'});const out=await insertMessage({cid:c.id,uid:req.user.id,text,kind:String(req.body.kind||'text'),fileUrl,fileType,fileName});io.to('conv:'+c.id).emit('message',out);res.json(out)}catch(e){console.error(e);res.status(500).json({error:'ارسال به پیام‌های ذخیره‌شده ناموفق بود'})}});
...
// SPA fallback: only GET/HEAD navigation requests should receive index.html.
